...
MyWiFiUdp::MyWiFiUdp() : udp_server(-1), server_port(0), remote_port(0), tx_buffer(0), tx_buffer_len(0), rx_buffer(0) {}

MyWiFiUdp::~MyWiFiUdp() {
  stop();
}

uint8_t MyWiFiUdp::begin(IPAddress address, uint16_t port) {
  stop();

  server_port = port;

  tx_buffer = (char *)malloc(MAXIMUM_UDP_PAYLOAD);
  if (!tx_buffer) {
...
uint8_t MyWiFiUdp::begin(uint16_t p) {
  return begin(IPAddress(), p);
}

uint8_t MyWiFiUdp::beginMulticast(IPAddress address, uint16_t p) {
...
int MyWiFiUdp::beginMulticastPacket() {
...
int MyWiFiUdp::beginPacket(IPAddress ip, uint16_t port) {
  remote_ip = ip;
  remote_port = port;
  return beginPacket();
}

int MyWiFiUdp::beginPacket(const char *host, uint16_t port) {
...
size_t MyWiFiUdp::write(uint8_t data) {
  if (tx_buffer_len == MAXIMUM_UDP_PAYLOAD) {
    endPacket();
    tx_buffer_len = 0;
  }
...
size_t MyWiFiUdp::write(const uint8_t *buffer, size_t size) {
  size_t i;
  for (i = 0; i < size; i++) {
    write(buffer[i]);
  }
...
void MyWiFiUdp::flush() {
  clear();
}

int MyWiFiUdp::parsePacket() {
  if (rx_buffer) {
    return 0;
  }
  ...
  char *buf = (char *)malloc(MAXIMUM_UDP_PAYLOAD);
  if (!buf) {
    return 0;
  }
  if ((len = recvfrom(udp_server, buf, MAXIMUM_UDP_PAYLOAD, MSG_DONTWAIT, (struct sockaddr *)&si_other_storage, (socklen_t *)&slen)) == -1) {
    free(buf);
...
int MyWiFiUdp::read(unsigned char *buffer, size_t len) {
  return read((char *)buffer, len);
}

int MyWiFiUdp::read(char *buffer, size_t len) {
  if (!rx_buffer) {
    return 0;
  }
...
IPAddress MyWiFiUdp::remoteIP() {
  return remote_ip;
}

uint16_t MyWiFiUdp::remotePort() {
  return remote_port;
}
